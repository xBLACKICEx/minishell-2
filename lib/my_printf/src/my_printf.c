...
int flags(char const *format, int *i, va_list list)
{
    ...
    for (; format[j] != '\0' && cmpc(sp, format[j], flag) != 0; j++) {
        ...
        if (cmpc(sp, format[j], flag) == 0) break;
    }
...
int my_printf(char const *format, ...)
{
    ...
    for (int i = 0; format[i] != '\0'; i++) {
        while (format[i] == '%') res = flags(format, &i, list);
        if (format[i] == '\0') break;
        write(MYSTDOUT, &format[i], 1);
    }
    va_end(list);
    return (res);
}

int my_printerr(char const *format, ...)
{
    va_list arg;
    int done;
    MYSTDOUT = 2;
